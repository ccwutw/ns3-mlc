...
NS_LOG_COMPONENT_DEFINE ("lte-alloc-dac");
...
{
  std::vector<uint32_t> band = {6,12,25,50,75,100};  //1.25MHz, 3.0MHz, 5.0MHz, 10.0MHz, 15.0MHz, 20.0MHz
  std::vector<uint32_t> size = {1,2,2,3,4,4};        //根据带宽不同，一个rbg对应的prb数不同
...
list<double> transRight;  //统计一段时间内，每个tti正确传输的数据量
list<double> allToTrans;  //统计一段时间内，每个tti传输的总数据量
double remainToTrans = 0.0; ///记录当前tti待传但未参与调度的数据
struct rParameters {
  //map的key都为rnti
  uint32_t cellid;
...
/*
  Define action space
  用于gym从ns3获取动作空间
  未使用此函数，但是space的大小决定了ns3和gym之间可传输数据量的多少
*/
Ptr<OpenGymSpace>
...
void PrintRlcStat()
{
  cout << "eNB\tRNTI\tLCID\t时延tx\tTXLen\t时延retx\tReTxLen" << std::endl;
  cout << "===============================================================" << std::endl;
  for (uint16_t i = 0; i < enbDevs.GetN(); i++)
  {
    ...
    for (auto it = pff->m_rlcBufferReq.begin (); it != pff->m_rlcBufferReq.end (); it++)
    {
      ...
      cout << rrp.m_rlcRetransmissionHolDelay << "\t" << rrp.m_rlcRetransmissionQueueSize << std::endl;
    }
  }
}
...
{
  stepCounter += 1;
  cout << "----------------------以上为step: " << stepCounter-1 << "时刻----------------------" << std::endl;
  cout << std::endl;
  cout << "----------------------以下为step: " << stepCounter << "时刻----------------------" << std::endl;
  cout << "OBS for step: " << stepCounter << " 调度前的RlcBuffer  " << Simulator::Now ().GetMilliSeconds() << std::endl;
  ...
  list<list<int>> ls;
  cout << "eNB\tRNTI\tLCID\tQCI\tCQI\t时延tx\tTXLen\t时延retx\tReTxLen" << std::endl;
  cout << "===============================================================" << std::endl;
  for (uint16_t i = 0; i < enbDevs.GetN(); i++)
  {
    ...
    for (auto it = pff->m_rlcBufferReq.begin (); it != pff->m_rlcBufferReq.end (); it++)
    {
      ...
      try{//尝试从高层m_higherLayerSelected获取各个rbg上的cqi，比较获得最差cqi，若没有则默认使用p10模式的cqi
      ...
      catch(std::out_of_range)//没有m_higherLayerSelected的cqi 进入异常处理
      {
    ...
    ls.sort (); //依等待传输的数据量排序，从小到大
    ls.reverse();//从大到小

  }

  uint16_t num_ue = 0; //统计总的业务请求数
  vector<uint32_t> unumpercell(enbDevs.GetN(),0);//统计各enb下业务请求数
  ...
  //进行补0
  if (num_ue <= enbDevs.GetN()*numOfrbg)
  {
    for (uint16_t i = 0; i < enbDevs.GetN()*numOfrbg - num_ue; i++)
      {
        box->AddValue (0);
        box->AddValue (0);
        box->AddValue (0);
...
MyExecuteActions (Ptr<OpenGymDataContainer> action)
{
  ...
  cout << std::endl;
  NS_LOG_UNCOND ("(" << stepCounter << "时刻)Action: " << tu_ac << std::endl);
  //以字符串的形式将动作传递到各个波束（eNB）进行调度
  cout << std::endl;
  cout << "(" << stepCounter << "时刻)调度结果: " << std::endl;
  for (uint16_t i = 0; i < enbDevs.GetN(); i++) {
...
MyGetReward (void)
{
  ...
  */
  ...
  if(isFirstWrite)
  {
    dataFile1.open("./scratch/lte-alloc-dac/throughoutput.txt", ofstream::trunc);
    dataFile2.open("./scratch/lte-alloc-dac/throughoutputrate.txt",  ofstream::trunc);
    dataFile1.close();
    dataFile2.close();
    isFirstWrite = false;
  }

  dataFile1.open("throughoutput.txt", ofstream::app);
  dataFile2.open("throughoutputrate.txt", ofstream::app);
  // 朝TXT文档中写入数据
...
main (int argc, char *argv[])
{
  ...
  double lambda = 50;//泊松分布参数，即到达率，单位时间内平均到达的业务数量
